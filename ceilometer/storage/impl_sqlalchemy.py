...
class Connection(base.Connection):
    ...
    NODE_UUID =''
    NODE_INFO = {}
    MESSGE_GENNERATED_TIME = datetime.datetime(2014,1,1,1,1)
    ...
    '''every time when we start the service ,we get the node info first,
        it include the information about the time of the hardware_ipmi_sel_list record datetime'''   
    def get_node_info(self):
        engine = self._engine_facade.get_engine()
        with engine.begin() as conn:
            trait = models.Trait.__table__
            nodes = conn.execute(sa.select([trait.c.t_string]).where(trait.c.trait_type_id==1))
            if nodes is None:
                return
            for node in nodes:  
                events = conn.execute(sa.select([trait.c.event_id]).where
                                      (trait.c.t_string ==node['t_string'])).fetchall() 
                value = [event['event_id'] for event in events]               
                last_date = conn.execute(sa.select([func.max(trait.c.t_string)]).where
                                         (sa.and_(trait.c.event_id.in_(value),
                                                  trait.c.trait_type_id==4))).first()                                                                             
                name = last_date.keys()[0]
                LOG.error("@@@@@@@@@@@@@@@@@the datetime is %s"%last_date[name])
                Connection.NODE_INFO[node['t_string']]=datetime.datetime.strptime(last_date[name],'%m/%d/%Y %H:%M:%S')
    ...
    def record_metering_data(self, data):
        ...
        :param data: a dictionary such as returned by
                     ...
                     LOG.error("************we are recording data %s"%data)
        """
    ...
    def record_events(self, event_models):
        ...
        for event_model in event_models:
            event = None
            ''''''
            if(event_model.event_type=='hardware.ipmi.sel.list'):
                event_dict = event_model.as_dict()
                for trait in event_dict['traits']:
                    if(trait['name']=='node_uuid' ):
                        event_host = trait['value']
                    if(trait['name']=='event_timestamp'):
                        event_timestamp = trait['value']
                time_in_event = datetime.datetime.strptime(event_timestamp,'%m/%d/%Y %H:%M:%S')  
                if(not Connection.NODE_INFO.has_key(event_host) ):
                    Connection.MESSGE_GENNERATED_TIME = event_timestamp 
                    Connection.NODE_UUID = event_host
                    Connection.NODE_INFO[event_host]=time_in_event

                if(Connection.NODE_UUID==event_host and Connection.MESSGE_GENNERATED_TIME==event_timestamp):
                    if(time_in_event>Connection.NODE_INFO[event_host]):
                         Connection.NODE_INFO[event_host] = time_in_event

                else:
                    Connection.NODE_UUID = ''
                    if(time_in_event < Connection.NODE_INFO[event_host] or 
                        time_in_event == Connection.NODE_INFO[event_host]):
                        continue
                    else:
                        Connection.NODE_INFO[event_host] = time_in_event
            else:
