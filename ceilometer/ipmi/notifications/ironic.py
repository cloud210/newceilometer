...
class InvalidSensorData(ValueError):
    pass
'''class InvalidSelData(ValueError):
    pass
class SelNotification(plugin.NotificationBase):
    event_types = ['hardware.ipmi.sel.list']
    @staticmethod
    ...
    def _package_payload(self, message, payload):
        ...
        info = {'publisher_id': message['payload']['node_uuid'],
    ...
    def process_notification(self, message):
        ...
        """
        LOG.error("I am in sellist process_notification")
        payloads = self._get_sample(message['payload'])
        for payload in payloads:
            LOG.error("data is %s******************"%payload['sensor']['number'])
            try:
                # Provide a fallback resource_id in case parts are missing.
                resource_id = 'missing id'
                try:
                    resource_id = '%s-%s'%(message['payload']['node_uuid'],
                         payload['sensor']['number'])
                    
                except KeyError as exc:
                    raise InvalidSelData('missing key in payload: %s' % exc)

                info = self._package_payload(message, payload)

               
                yield sample.Sample.from_notification(
                    name='hardware.ipmi.sel.list' ,
                    type=sample.TYPE_GAUGE,
                    unit=info['publisher_id'],
                    volume=None,
                ...
                LOG.warn(
                    'invalid sel data for %(resource)s: %(error)s' %
...
class SensorNotification(plugin.NotificationBase):
    ...
    event_types = ['hardware.ipmi.Temperature','hardware.ipmi.Current','hardware.ipmi.Fan','hardware.ipmi.Voltage']
    metric = None
