...
class EventsNotificationEndpoint(object):
    ...
    def _package_payload(self, message, payload):
        notification = {}
        notification['event_type']=message['event_type']
        notification['message_id']= uuid.uuid4().hex
        payload['node_uuid']=message['payload']['node_uuid']
        if(payload.has_key('node_ip')):
            payload['node_ip']=message['payload']['node_ip']
    ...
    def process_notification(self, notification):
        ...
        payloads = self._get_sample(notification['payload'])
        if payloads is None:
            self._process_notification(notification)
        else:
            for payload in payloads:
                notification = self._package_payload(notification, payload)
                self._process_notification(notification)
